<script>
  ...
  $(".ticket-status").on('change', function(){
    ...
    window.location.href = "<?= base_url('ticket/');?>" + status;
  });
  ...
  $('.add-ticket').on('click', function() {
      ...
      $('.modal-body form').attr('action', '<?= base_url("add-ticket");?>');
  ...
  $('.edit-ticket').on('click', function() {
    ...
    $('.modal-body form').attr('action', '<?= base_url("edit-ticket");?>');

    const ticket_code = $(this).data('id');

    $.ajax({
        url: '<?= base_url("get-ticket");?>',
        data: {
                id: ticket_code, 
